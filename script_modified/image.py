...
import numpy
from numpy import float as Float, sum, ravel, ones
...
class FastImageSampler(ImageSampler):
    ...
    def __call__(self, image, x, y, sheet_xdensity, sheet_ydensity, width=1.0, height=1.0):
        ...
        return numpy.array(im,dtype=Float)
...
class ImcImage(GenericImage):
    ...
    filename = param.Filename(default='/mnt/c/Users/jiboonne/Documents/Python/topographica/imk00001.imc',precedence=0.9,doc="""
        File path (can be relative to Param's base path) to a bitmap image.
        The image can be in any format accepted by PIL, e.g. PNG, JPG, TIFF, or PGM.
        """)
    ...
    def __init__(self, **params):
        ...
        avoided.
        """
        super(ImcImage,self).__init__(**params)
        self.last_filename = None
    ...
    def _get_image(self,p):
        ...
        if p.filename!=self.last_filename or self._image is None:
            self.last_filename=p.filename
            import array
            with open(p.filename, 'rb') as handle:
                s = handle.read()
            arr = array.array('H',s)
            arr.byteswap()
            img = numpy.array(arr, dtype='uint16').reshape(1024,1536) 
            self._image = Image.fromarray(img)
        return self._image
