...
class CustomArray(PatternGenerator):
    pattern_array = param.Array(default=numpy.ones(10, dtype=float))
    
    def function(self,p):
        result = p.pattern_array
        return result
...
def wrap(lower, upper, x):
    ...
    range_=upper-lower
    return lower + math.fmod(x-lower + 2*range_*(1-floor(x/(2*range_))), range_)
